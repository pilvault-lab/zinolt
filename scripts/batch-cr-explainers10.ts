...
const TMP = join(tmpdir(), "zinolt-cr-explainers10");
const SRC_CACHE = join(TMP, "sources");
const SUB_CACHE = join(TMP, "subs");
const REPORT = join(OUT_DIR, "cr-explainers-run-report.md");
...
const CLIPS: Clip[] = [
  { n: "11", videoId: "r48p_O23Dsw", hook: "Why money isn't worthless (yet)",       slug: "why-money-isnt-worthless",  sourceTitle: "Why Money isn't Worthless...yet." },
  { n: "12", videoId: "19NJvHiUgTQ", hook: "$500B stolen with tax loopholes",       slug: "tax-loopholes",             sourceTitle: "How Companies Steal $500 Billion A Year with Tax Loopholes" },
  { n: "13", videoId: "trR9J_w6xj4", hook: "How casinos launder $383M a day",       slug: "casinos-launder",           sourceTitle: "How Casinos Launder $383 Million per Day" },
  { n: "14", videoId: "UX50vSmyz8Y", hook: "Who spends the US national debt",       slug: "who-spends-us-debt",        sourceTitle: "Who Spends the U.S. National Debt? ($35 Trillion)" },
  { n: "15", videoId: "lQ-IT8LjC0Y", hook: "What are meme stocks",                  slug: "meme-stocks",               sourceTitle: "What are Meme Stocks? The History of GME and Meme Stocks" },
  { n: "16", videoId: "3xXUQEvf8v0", hook: "Financial terms explained simply",      slug: "financial-terms",           sourceTitle: "Financial Terms Explained as Simply as Possible" },
  { n: "17", videoId: "g2pIec0NKFk", hook: "What is a bear market",                 slug: "what-is-bear-market",       sourceTitle: "What Is a Bear Market? (And How Do They Work?)" },
  { n: "18", videoId: "28nPS7Sdrq8", hook: "How is crypto taxed",                   slug: "how-crypto-taxed",          sourceTitle: "How is Crypto Taxed? (Bitcoin, Ethereum, NFTs)" },
  { n: "19", videoId: "TI7DpqpnbO4", hook: "How the debt ceiling works",            slug: "debt-ceiling",              sourceTitle: "How the Debt Ceiling & Debt Limit Works" },
  { n: "20", videoId: "0P_ZwZ1Vyds", hook: "What is a penny stock",                 slug: "penny-stock",               sourceTitle: "What Is a Penny Stock? (and should you buy one?)" },
];
...
function pickWindow(cues: Cue[]): Window | null {
  ...
  const TARGET = 120;
  const INTRO_SKIP = 3;
  const CTA_BUFFER = 2;  // seconds after CTA block ends before we start
...
async function runClip(c: Clip): Promise<void> {
  ...
  const outName = `cr-${c.n}-${c.slug}.mp4`;
...
async function main() {
  ...
  console.log(`\nDone. Report: outputs/money-simplified/cr-explainers-run-report.md`);
}
